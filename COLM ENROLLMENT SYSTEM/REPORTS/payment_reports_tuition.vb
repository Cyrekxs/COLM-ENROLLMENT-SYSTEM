Public Class payment_reports_tuition
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Using conn As New SqlConnection(StringConnection)
            conn.Open()
            Using comm As New SqlCommand("SELECT DISTINCT STUDENT_NUMBER,(SELECT TOP 1 LASTNAME + CASE EXTENSION_NAME WHEN 'N.A' THEN '' WHEN 'NONE' THEN '' WHEN '' THEN '' ELSE ' ' + EXTENSION_NAME END + ' ' + FIRSTNAME + CASE MIDDLENAME WHEN 'N.A' THEN '' WHEN 'NONE' THEN '' WHEN '' THEN '' ELSE ' ' + MIDDLENAME END FROM TBL_STUDENT_INFORMATION WHERE STUDENT_NUMBER = TBL_COLLEGE_PAYMENT.STUDENT_NUMBER) AS STUDENT_NAME,FEE_TYPE,RECIEPT_NUMBER,RECIEPT_STATUS,SUM(AMOUNT_COLLECTED) AS AMOUNT_COLLECTED,RECEIVER,CONVERT(char(10),DATE_RECEIVED,126) AS DATE_RECIEVED,EDUCATION_LEVEL FROM TBL_COLLEGE_PAYMENT WHERE RECIEPT_NUMBER LIKE @reciept_no AND FEE_STATUS = 'TUITION FEE' AND FEE_TYPE LIKE @transaction_type AND RECIEPT_STATUS LIKE @reciept_status AND EDUCATION_LEVEL LIKE @education_level AND DATE_RECEIVED BETWEEN @date1 AND @date2 GROUP BY STUDENT_NUMBER,FEE_TYPE,RECIEPT_NUMBER,RECIEPT_STATUS,RECEIVER,CONVERT(char(10),DATE_RECEIVED,126),EDUCATION_LEVEL ORDER BY RECIEPT_NUMBER ASC", conn)
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        ...
        Dim DR As DataRow
        Dim param_ReportTitle As ReportParameter = New ReportParameter("ReportTitle", "TOTAL COLLECTIONS IN TUITION FEE")
