Public Class payment_reports_nonstudent

    Private Sub LoadFeeDescription()
        Using conn As New SqlConnection(StringConnection)
            conn.Open()
            Using comm As New SqlCommand("SELECT DISTINCT Fee_Description FROM vw_nonstudent_collection_reports WHERE AccountType = 'NON STUDENT' ORDER BY Fee_Description ASC", conn)
                Using reader As SqlDataReader = comm.ExecuteReader()
                    While reader.Read()
                        txtRecieptNo.AutoCompleteCustomSource.Add(reader("Fee_Description"))
                    End While
                End Using
            End Using
        End Using
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        Using conn As New SqlConnection(StringConnection)
            conn.Open()
            Using comm As New SqlCommand("SELECT * FROM vw_nonstudent_collection_reports WHERE AccountType = @education_level AND Fee_Type = @FeeType AND Reciept_Status = @ReceiptStatus AND DATE_RECEIVED BETWEEN @date1 AND @date2 AND AccountName + Fee_Description LIKE @search", conn)
                comm.Parameters.AddWithValue("@search", "%" & txtRecieptNo.Text & "%")
    ...
    Private Sub payment_reports_nonstudent_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadFeeDescription()
    End Sub
End Class
