Public Class frm_pre_elementary_assessment_browse_student
    ...
    Public Sub Load_Students()
        Using conn As New SqlConnection(StringConnection)
            conn.Open()
            Using comm As New SqlCommand("SELECT STUDENT_NUMBER,LASTNAME + CASE EXTENSION_NAME WHEN 'N.A' THEN '' ELSE ' ' + EXTENSION_NAME END +  ', ' + FIRSTNAME + CASE MIDDLENAME WHEN 'N.A' THEN '' ELSE ' ' + MIDDLENAME END AS STUDENT_NAME,(SELECT COURSECODE FROM TBL_STUDENT_REGISTERED WHERE STUDENT_NUMBER = TBL_STUDENT_INFORMATION.STUDENT_NUMBER AND ACADEMIC_YEAR = @ay) AS COURSE_CODE,(SELECT YRLVL FROM TBL_STUDENT_REGISTERED WHERE STUDENT_NUMBER = TBL_STUDENT_INFORMATION.STUDENT_NUMBER AND ACADEMIC_YEAR = @ay) AS YRLVL,(SELECT SECTIONCODE FROM TBL_STUDENT_REGISTERED WHERE STUDENT_NUMBER = TBL_STUDENT_INFORMATION.STUDENT_NUMBER AND ACADEMIC_YEAR = @ay) AS SECT_CODE,(SELECT STUDENT_NUMBER FROM TBL_COLLEGE_ASSESSMENT_SUMMARY WHERE STUDENT_NUMBER = TBL_STUDENT_INFORMATION.STUDENT_NUMBER AND ACADEMIC_YR = @ay) AS AssessmentStatus,(SELECT PullOutStatus FROM TBL_COLLEGE_ASSESSMENT_SUMMARY WHERE STUDENT_NUMBER = TBL_STUDENT_INFORMATION.STUDENT_NUMBER AND ACADEMIC_YR = @ay AND ACADEMIC_SEM = @sem) AS PullOutStatus FROM TBL_STUDENT_INFORMATION WHERE EXISTS (SELECT STUDENT_NUMBER FROM TBL_STUDENT_REGISTERED WHERE STUDENT_NUMBER = TBL_STUDENT_INFORMATION.STUDENT_NUMBER AND EDUCATION_LEVEL = @education_level AND ACADEMIC_YEAR = @ay) AND LASTNAME LIKE @search ORDER BY STUDENT_NAME ASC", conn)
