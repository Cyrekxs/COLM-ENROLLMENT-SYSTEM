                ...
                Try
                    Using comm As New SqlCommand("SELECT * FROM TBL_COLLEGE_PAYMENT WHERE RECIEPT_NUMBER = @rn", conn, t)
                        ...
                        If result = 0 Then
                            If MsgBox("Are you sure you want to proceed this transaction?", MsgBoxStyle.Question + MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                                For i = 0 To DataGridView3.Rows.Count - 1
                                    If CDbl(DataGridView3.Rows(i).Cells(3).Value) > 0 Then
                                        Using comm1 As New SqlCommand("INSERT INTO TBL_COLLEGE_PAYMENT VALUES(@sn,@rn,@r_status,@fee_type,@fee_code,@fee_status,@reason,@amount_received,@amount_collected,@reciever,GETDATE(),@ay,@sem,@education_level)", conn, t)
                            ...
                            End If
                        Else
                            MsgBox("CM#: " & txtReceipt.Text & " is already exists!", MsgBoxStyle.Critical)
                        End If
                    End Using
