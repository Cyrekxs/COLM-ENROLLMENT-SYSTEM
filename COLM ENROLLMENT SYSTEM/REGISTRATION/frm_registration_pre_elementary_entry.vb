Public Class frm_registration_pre_elementary_entry
    ...
    Public Sub LoadStudents()
        ...
        Using conn As New SqlConnection(StringConnection)
            ...
            SQLQuery = "SELECT ID,TBL_STUDENT_INFORMATION.STUDENT_NUMBER,LASTNAME,FIRSTNAME,MIDDLENAME,EXTENSION_NAME FROM TBL_STUDENT_INFORMATION WHERE NOT EXISTS (SELECT STUDENT_NUMBER FROM TBL_STUDENT_REGISTERED WHERE STUDENT_NUMBER = TBL_STUDENT_INFORMATION.STUDENT_NUMBER AND ACADEMIC_YEAR = @ay AND EDUCATION_LEVEL = @education_level) AND LASTNAME + ', ' + FIRSTNAME LIKE @search AND TBL_STUDENT_INFORMATION.EDUCATION_LEVEL = @education_level ORDER BY LASTNAME,FIRSTNAME ASC"
    ...
    Private Sub frm_registration_highschool_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadStudents()
        Load_YearLvls("PRE ELEMENTARY", "PRE ELEMENTARY", cmbYear)
    End Sub

    Private Sub cmbYear_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmbYear.SelectedIndexChanged
        Load_Sections("PRE ELEMENTARY", "PRE ELEMENTARY", cmbYear.Text, cmbSection)
    End Sub
    ...
    Private Sub btnRegister_Click(sender As Object, e As EventArgs) Handles btnRegister.Click
        ...
        Using conn As New SqlConnection(StringConnection)
            conn.Open()
            Using comm As New SqlCommand("SELECT * FROM TBL_STUDENT_REGISTERED WHERE EDUCATION_LEVEL = @education_level AND STUDENT_NUMBER = @sn AND ACADEMIC_YEAR = @ay AND ACADEMIC_SEM = @sem", conn)
                comm.Parameters.AddWithValue("@education_level", EducationLevel)
                comm.Parameters.AddWithValue("@sn", txtStudentNo.Text)
                comm.Parameters.AddWithValue("@ay", Academic_Year)
                comm.Parameters.AddWithValue("@sem", Academic_Sem)
                If Val(comm.ExecuteScalar) = 0 Then
                    Using commInsert As New SqlCommand("INSERT INTO TBL_STUDENT_REGISTERED VALUES(@education_level,@sn,@course_code,@yrlvl,@section_code,@enrollment_status,@ay,@sem,@registeredby,GETDATE(),'ACTIVE')", conn)
                        commInsert.Parameters.AddWithValue("@sn", txtStudentNo.Text)
                        commInsert.Parameters.AddWithValue("@education_level", EducationLevel)
                        commInsert.Parameters.AddWithValue("@course_code", "PRE ELEMENTARY")
                        ...
                        frm_registered_pre_elementary_students.MdiParent = frm_main
                        frm_registered_pre_elementary_students.Show()
                        frm_registered_pre_elementary_students.Top = 0
                        frm_registered_pre_elementary_students.Left = 0
                        Me.Close()
                    End Using
                End If
            End Using
