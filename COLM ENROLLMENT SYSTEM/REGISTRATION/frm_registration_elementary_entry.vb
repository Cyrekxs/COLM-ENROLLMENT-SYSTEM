Public Class frm_registration_elementary_entry
    ...
    Public Sub LoadStudents()
        ...
        Using conn As New SqlConnection(StringConnection)
            ...
            Using comm As New SqlCommand(SQLQuery, conn)
                comm.Parameters.AddWithValue("@education_level", "ELEMENTARY")
    ...
    Private Sub btnRegister_Click(sender As Object, e As EventArgs) Handles btnRegister.Click
        ...
        Using conn As New SqlConnection(StringConnection)
            conn.Open()
            Using comm As New SqlCommand("SELECT * FROM TBL_STUDENT_REGISTERED WHERE EDUCATION_LEVEL = 'ELEMENTARY' AND STUDENT_NUMBER = @sn AND ACADEMIC_YEAR = @ay AND ACADEMIC_SEM = @sem", conn)
                comm.Parameters.AddWithValue("@sn", txtStudentNo.Text)
                comm.Parameters.AddWithValue("@ay", Academic_Year)
                comm.Parameters.AddWithValue("@sem", "1ST SEMESTER")
                If Val(comm.ExecuteScalar) = 0 Then
                    Using commInsert As New SqlCommand("INSERT INTO TBL_STUDENT_REGISTERED VALUES('ELEMENTARY',@sn,@course_code,@yrlvl,@section_code,@enrollment_status,@ay,@sem,@registeredby,GETDATE(),'ACTIVE')", conn)
                        commInsert.Parameters.AddWithValue("@sn", txtStudentNo.Text)
                        commInsert.Parameters.AddWithValue("@course_code", "ELEMENTARY")
                        ...
                        frm_registered_elementary_students.MdiParent = frm_main
                        frm_registered_elementary_students.Show()
                        frm_registered_elementary_students.Top = 0
                        frm_registered_elementary_students.Left = 0
                        Me.Close()
                    End Using
                End If
            End Using
